...
def tsv_2_HGVSG():
    ...
    with open('tsv_input.tsv','r',newline='') as fin,open('tsv_output.tsv','w',newline='') as fout:
        ...
        writer = csv.DictWriter(fout,delimiter='\t',fieldnames=[' HGVSG' , 'PRIMARY_SITE'],extrasaction='ignore')
        writer.writeheader()
        for row in reader:
            writer.writerow(row)
...
def HGVSG_2_CRAVAT():
    """
    Writes the new CRAVAT input file from the 'tsv_output.tsv' file. Contains 7 columns.
    """
    with open('tsv_output.tsv','r',newline='') as fin,open('oldCRAVAT_input.tsv','w',newline='') as fout:
        reader = csv.DictReader(fin,delimiter='\t')
